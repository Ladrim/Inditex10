...
public class ProductEntityInH2Adapter
{
    private final DateFormatter dateFormatter;

    public ProductEntityInH2Adapter(DateFormatter dateFormatter)
    {
    ...
    public PriceInH2Entity toEntity(Product product, Price price)
    {
        ...
        String currency = price.getMoney().getCurrencyType().getValue();
        Float amount  = price.getMoney().getAmount();
        String productId = product.getId().getValue().toString();

        return new PriceInH2Entity(id, brandId, startDate, endDate, priority, currency, amount, productId);
    }

    public ProductId toDomain(String productId)
    {
